    ...
    return (
        <>
            <section className="py-10 bg-[#F4F8FB]">
                <div className="container px-2 mx-auto md:px-5">
                                    ...
                                    {categories.length && categories.map(cat => (
                                        <li className={`my-1 ${(cat.children_recursive.length > 0) ? 'has-children' : ''} ${((openToggle === cat.id)) ? 'active' : ''}`} key={cat.id}>
                                            <div className="flex justify-between items-center gap-4 group">
                                                <Link href={`/product-category/${cat.slug}`} className={`text-sm font-semibold py-2 flex justify-between w-full group-hover:text-primary xl:text-sm max-w-[calc(100%-41px)] text-dark`}>{cat.name} </Link>
                                                    ...
                                                    {cat.children_recursive.map((child) => (
                                                        (child.products_count > 0 &&
                                                            <li className={`my-1 ${(child.children_recursive.length > 0) ? 'has-children' : ''} ${((openToggle === child.id)) ? 'active' : ''}`} key={child.id}>
                                                                <div className="flex justify-between items-center gap-4 group">
                                                                    <Link href={`/product-category/${cat.slug}/${child.slug}`} className={`text-sm font-semibold py-2 flex justify-between w-full group-hover:text-primary xl:text-sm max-w-[calc(100%-41px)] text-dark`}>{child.name} {(child.products_count > 0) ? <span>({child.products_count})</span> : ''}</Link>
                                                                        ...
                                                                        {child.children_recursive.map((value) => (
                                                                            (value.products_count ?
                                                                                <li className={`my-1 ${(value.children_recursive.length > 0) ? ' has-children' : ''} ${((openToggle === value.id)) ? 'active' : ''}`} key={value.id}>
                                                                                    <div className="flex justify-between items-center gap-4 group">
                                                                                        <Link href={`/product-category/${cat.slug}/${child.slug}/${value.slug}`} className={`text-xs font-semibold py-2 flex justify-between w-full group-hover:text-primary xl:text-sm max-w-[calc(100%-41px)] text-dark`}>{value.name} {(value.products_count > 0) ? <span>({value.products_count})</span> : ''}</Link>
                        ...
                        <div className="col-span-10 lg:col-span-7">
                            ...
                            <div className="bg-[#fff] shadow-sm  p-4 px-5 mb-5 xl:mb-6">
                                <div className="flex flex-wrap gap-2 items-center">
                                    <h5 className="text-sm font-semibold xl:text-base">Sort By : </h5>
                            ...
                            {brandDetail?.description &&
                                <div className="bg-[#fff] shadow-sm  p-4 px-5 mb-5 xl:mb-6">
                                    <div className={`grid grid-cols-3 gap-5 mb-5 ${!read ? "before-bokeh" : null} relative`}>
                                        <div className="img-area group overflow-hidden">
                                            <Image src={brandDetail?.image_url} alt={brandDetail?.name} />
                                        </div>
                                        <div className="text-area col-span-2">
                                            <div className={`${read ? "h-auto" : "h-[275px]"} overflow-hidden transition-all duration-300`}>
                                                <div dangerouslySetInnerHTML={{ __html: brandDetail?.description }}></div>
                                                <h1 className="text-xl text-dark mb-3"><strong className="font-semibold">Laboratory Consumables – The Reliable Disposable Products for Any Lab Need</strong></h1>
                                                <p className="text-sm leading-6 font-medium text-justify text-[#33333395] mb-2">At Laboratory Disposable Products (LDP), we understand that the secret to any successful laboratory is the availability of high-quality, reliable laboratory consumables. These products are central to providing smooth workflows, avoiding contamination hazards, and providing accurate results in scientific research, diagnostics, education, and industrial applications.</p>
                                                <p className="text-sm leading-6 font-medium text-justify text-[#33333395] mb-2">At Laboratory Disposable Products (LDP), we understand that the secret to any successful laboratory is the availability of high-quality, reliable laboratory consumables. These products are central to providing smooth workflows, avoiding contamination hazards, and providing accurate results in scientific research, diagnostics, education, and industrial applications.</p>
                                                <p className="text-sm leading-6 font-medium text-justify text-[#33333395] mb-2">At Laboratory Disposable Products (LDP), we understand that the secret to any successful laboratory is the availability of high-quality, reliable laboratory consumables. These products are central to providing smooth workflows, avoiding contamination hazards, and providing accurate results in scientific research, diagnostics, education, and industrial applications.</p>
                                                <p className="text-sm leading-6 font-medium text-justify text-[#33333395] mb-2">At Laboratory Disposable Products (LDP), we understand that the secret to any successful laboratory is the availability of high-quality, reliable laboratory consumables. These products are central to providing smooth workflows, avoiding contamination hazards, and providing accurate results in scientific research, diagnostics, education, and industrial applications.</p>
                                            </div>
                                    ...
                                    <button className={`${read ? "bg-secondary" : "bg-primary"} text-white flex gap-1 font-semibold px-2 py-2 cursor-pointer hover:bg-secondary`}
                                        onClick={() => {
                                    ...
                                    <li className="inline-flex items-center">
                                        ...
                                        <li>
                                            <div className="flex items-center">
                                                ...
                                                <span className="ms-1 text-base font-medium text-gray-500 md:ms-2 dark:text-gray-400">{brandDetail?.name}</span>
                                            </div>
                                        </li>
                                </ol>
                            </nav>
